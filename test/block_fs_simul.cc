...
// block_fs_fflush(stdout);
// block_fs_fprintf(stdout, "%s: Process %d(%s): FAILED in %s, %s: %s\n",
...
// block_fs_fprintf(stdout, "%s: Process %d(%s): FAILED, %s: %s\n",
...
inline void begin(const std::string &str) {
  if (verbose > 0 && rank == 0) {
    gettimeofday(&t1, NULL);
    // block_fs_fprintf(stdout, "%s:\tBeginning %s\n", timestamp(), str);
    // block_fs_fflush(stdout);
  }
}

inline void end(const std::string &str) {
  double elapsed;
  ...
  if (verbose > 0 && rank == 0) {
    ...
    elapsed =
        ...
        (double)1000000;
    if (elapsed >= 60) {
      // block_fs_fprintf(stdout, "%s:\tFinished %-15s(%.2f min)\n",
      // timestamp(), str, elapsed / 60);
    } else {
      // block_fs_fprintf(stdout, "%s:\tFinished %-15s(%.3f sec)\n",
...
void remove_file_or_dir(char *name) {
  ...
  if (block_fs_stat(name, &statbuf) != -1) {
    if (S_ISREG(statbuf.st_mode)) {
      printf("stale file found\n");
      if (block_fs_unlink(name) == -1) {
...
char *create_files(const std::string &prefix, int filesize, int shared) {
  ...
  if (rank == 0) {
    for (i = 0; i < (shared ? 1 : size); i++) {
      sprintf(filename, "%s/%s.%d", testdir, prefix.c_str(), i);
      remove_file_or_dir(filename);
      if ((fd = block_fs_create(filename, FILEMODE)) == -1) {
        sprintf(errmsg, "creat of file %s", filename);
        FAIL(errmsg);
      }
      if (filesize > 0) {
        if (block_fs_lseek(fd, filesize - 1, SEEK_SET) == -1) {
          sprintf(errmsg, "lseek in file %s", filename);
          FAIL(errmsg);
        }
        if (block_fs_write(fd, &zero, 1) == -1) {
          sprintf(errmsg, "write in file %s", filename);
          FAIL(errmsg);
        }
      }
      if (block_fs_close(fd) == -1) {
  ...
  if (shared)
    sprintf(filename, "%s/%s.0", testdir, prefix.c_str());
  else
    sprintf(filename, "%s/%s.%d", testdir, prefix.c_str(), rank);

  return filename;
}

void remove_files(const std::string &prefix, int shared) {
  char filename[1024];
  int i;
  ...
  if (rank == 0) {
    for (i = 0; i < (shared ? 1 : size); i++) {
      sprintf(filename, "%s/%s.%d", testdir, prefix.c_str(), i);
      // /*printf("Removing file %s\n", filename); block_fs_fflush(stdout);*/
      if (block_fs_unlink(filename) == -1) {
        FAIL("unlink failed");
      }
    }
  }
...
char *create_dirs(const std::string &prefix, int shared) {
  ...
  if (rank == 0) {
    for (i = 0; i < (shared ? 1 : size); i++) {
      sprintf(dirname, "%s/%s.%d", testdir, prefix.c_str(), i);
      remove_file_or_dir(dirname);
      if (block_fs_mkdir(dirname, DIRMODE) == -1) {
  ...
  if (shared)
    sprintf(dirname, "%s/%s.0", testdir, prefix.c_str());
  else
    sprintf(dirname, "%s/%s.%d", testdir, prefix.c_str(), rank);

  return dirname;
}

void remove_dirs(const std::string &prefix, int shared) {
  char dirname[1024];
  int i;
...
char *create_symlinks(const std::string &prefix, int shared) {
  ...
  if (rank == 0) {
    for (i = 0; i < (shared ? 1 : size); i++) {
      ...
      sprintf(linkname, "%s/%s.%d", testdir, prefix.c_str(), i);
      remove_file_or_dir(linkname);
      if (block_fs_symlink(filename, linkname) == -1) {
        FAIL("symlink failed");
      }
    }
  }
  ...
  if (shared)
    sprintf(linkname, "%s/%s.0", testdir, prefix.c_str());
  else
    sprintf(linkname, "%s/%s.%d", testdir, prefix.c_str(), rank);

  return linkname;
}

void check_single_success(const std::string &testname, int rc, int error_rc) {
  int *rc_vec, i;
  int fail = 0;
  int pass = 0;
  ...
  // // // // MPI_Gather(&rc, 1, MPI_INT, rc_vec, 1, MPI_INT, 0,
  // MPI_COMM_WORLD);
  if (rank == 0) {
    for (i = 0; i < size; i++) {
    ...
    if (!((pass == 1) && (fail == size - 1))) {
      // block_fs_fprintf(stdout, "%s: FAILED in %s: ", timestamp(), testname);
      // if (pass > 1)
      // block_fs_fprintf(stdout, "too many operations succeeded (%d).\n",
      // pass);
      // else
      // block_fs_fprintf(stdout, "too many operations failed (%d).\n", fail);
      // block_fs_fflush(stdout);
...
void simul_open(int shared) {
  ...
  begin("test");
  if ((fd = block_fs_open(filename, O_RDWR)) == -1) {
    FAIL("open failed");
  }
  end("test");
...
void simul_file_stat(int shared) {
  ...
  begin("test");
  /* All stat the file */
  if (block_fs_stat(filename, &buf) == -1) {
    FAIL("stat failed");
  }
  end("test");
...
void simul_dir_stat(int shared) {
  ...
  if (block_fs_stat(dirname, &buf) == -1) {
    FAIL("stat failed");
  }
  end("test");
...
void simul_readdir(int shared) {
  ...
  // Seq_begin(MPI_COMM_WORLD, throttle);
  if ((dir = block_fs_opendir(dirname)) == NULL) {
  ...
  if ((dptr = block_fs_readdir(dir)) == NULL) {
    FAIL("readdir failed");
  }
  end("test");
...
void simul_statfs(int shared) {
  ...
  if (block_fs_statvfs(filename, &buf) == -1) {
    FAIL("statfs failed");
  }
  end("test");
...
void simul_lseek(int shared) {
  ...
  if (block_fs_lseek(fd, 1024, SEEK_SET) == -1) {
    FAIL("lseek failed");
    // MPI_Abort(MPI_COMM_WORLD, 1);
  }
  end("test");
...
void simul_read(int shared) {
  ...
  for (i = 1024; (i > 0) && (retry > 0); i -= fin, retry--) {
    if ((fin = block_fs_read(fd, buf, (size_t)i)) == -1) {
      FAIL("read failed");
    }
  }
  if ((retry == 0) && (i > 0)) FAIL("read exceeded retry count");
  end("test");
...
void simul_write(int shared) {
  ...
  if (block_fs_lseek(fd, rank * sizeof(int), SEEK_SET) == -1) {
  ...
  for (i = sizeof(int); (i > 0) && (retry > 0); i -= fin, retry--) {
    if ((fin = block_fs_write(fd, &rank, (size_t)i)) == -1) {
      FAIL("write failed");
    }
  }
  if ((retry == 0) && (i > 0)) FAIL("write exceeded retry count");
  end("test");
...
void simul_mkdir(int shared) {
  ...
  begin("test");
  /* All mkdir dirname */
  rc = block_fs_mkdir(dirname, DIRMODE);
  if (!shared) {
    if (rc == -1) {
      FAIL("mkdir failed");
    }
...
void simul_creat(int shared) {
  ...
  fd = block_fs_create(filename, FILEMODE);
  if (fd == -1) {
    FAIL("creat failed");
  }
  end("test");
...
void simul_rename(int shared) {
  ...
  const std::string testname = "simul_rename";

  begin("setup");
  oldfilename = create_files(testname, 0, shared);
  sprintf(newfilename, "%s/%s_new.%d", testdir, testname.c_str(), rank);
  if (rank == 0) {
    for (i = 0; i < (shared ? 1 : size); i++) {
      char buf[MAX_FILENAME_LEN];
      sprintf(buf, "%s/%s_new.%d", testdir, testname.c_str(), i);
      remove_file_or_dir(buf);
    }
  }
  end("setup");
  ...
  rc = block_fs_rename(oldfilename, newfilename);
  if (!shared) {
    if (rc == -1) {
      FAIL("stat failed");
    }
  ...
  begin("cleanup");
  if (rc == 0) {
    if (block_fs_unlink(newfilename) == -1) FAIL("unlink failed");
  }
  end("cleanup");
}
...
void simul_truncate(int shared) {
  ...
  if (block_fs_truncate(filename, 1024) == -1) {
...
void simul_readlink(int shared) {
  ...
  if (block_fs_readlink(linkname, buf, 1024) == -1) {
...
void simul_symlink(int shared) {
  ...
  rc = block_fs_symlink(filename, linkname);
  if (!shared) {
    if (rc == -1) {
      FAIL("symlink failed");
    }
...
void simul_fcntl_lock(int shared) {
  ...
  rc = block_fs_fcntl(fd, F_SETLK, &sf_lock);
  if (!shared) {
    if (rc == -1) {
      if (errno == ENOSYS) {
        if (rank == 0) {
          // block_fs_fprintf(stdout, "WARNING: fcntl locking not
          // supported.\n"); block_fs_fflush(stdout);
        }
      } else {
    ...
    if (rank == 0) {
      ...
      if (nosys == size) {
        // block_fs_fprintf(stdout, "WARNING: fcntl locking not supported.\n");
        // block_fs_fflush(stdout);
      } else if (!((pass == 1) && (fail == size - 1))) {
        // block_fs_fprintf(stdout,
  ...
  // Seq_begin(MPI_COMM_WORLD, throttle);
  if (!shared || rank == 0) {
    rc = block_fs_fcntl(fd, F_SETLK, &sf_unlock);
    if (rc == -1 && errno != ENOSYS) FAIL("fcntl unlock failed");
  }
  if (block_fs_close(fd) == -1) {
    FAIL("close failed");
  }
      ...
      if (!int_in_list(test, excl_list)) {
        for (j = 0; j < iterations; j++) {
          if (singlestep) {
            ...
            // block_fs_fgets(linebuf, 80, stdin);
          }
          if (rank == 0) {
            printf("%s: Running test #%d(iter %d): %s, %s mode.\n", timestamp(),
                   test, j, testlist[test % testcnt].name,
                   (test < testcnt) ? "shared" : "individual");
            // block_fs_fflush(stdout);
          }
          testlist[test % testcnt].function((test < testcnt) ? SHARED
                                                             : !SHARED);
          // // MPI_Barrier(MPI_COMM_WORLD);
        }
      }
